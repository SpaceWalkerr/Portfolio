...
import { TerminalSection, SectionHeader, TermButton, neonReveal } from './ui/terminal';
...
const Certifications = () => {
  ...
  return (
    <TerminalSection id="certifications" deep>
      <SectionHeader
        command="tail -f achievements.log"
        headline="achievements_log"
        standfirst="Validated skills and milestones, indexed by category — sixty-plus entries logged since 2023."
      />

      <div className="mb-10 flex flex-wrap gap-2 border-y border-neon-cyan/20 py-4">
        {categories.map((category) => (
          <button
            key={category}
            type="button"
            onClick={() => setActiveFilter(category)}
            className={`px-3.5 py-1.5 font-mono text-[9.5px] lowercase tracking-[0.08em] transition-colors ${
              activeFilter === category ? 'bg-neon-pink/90 text-void' : 'border border-neon-cyan/25 text-ink-mute hover:border-neon-cyan/50 hover:text-ink'
            }`}
          >
      ...
      {showFeatured && (
        <motion.div variants={neonReveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }} className="mb-14">
          <h3 className="mb-1 font-display text-xl font-bold uppercase tracking-wide text-ink">[PRIORITY] entries</h3>
          <p className="mb-6 font-mono text-xs text-ink-mute">The highest-impact log lines.</p>

          <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
            {notableAchievements.map((item, i) => (
              <div key={item.title} className="flex gap-4 border-t border-neon-cyan/25 pt-3">
                <span className="font-mono text-lg text-neon-pink">{`0${i + 1}`}</span>
                <div>
                  <h4 className="font-mono text-sm font-bold text-ink">{item.title}</h4>
          ...
          <div className="mt-10 divide-y divide-white/5 border-y border-neon-cyan/20">
            {featuredCerts.map((cert) => (
              ...
              >
                <div className="min-w-0">
                  <span className="font-mono text-[14px] text-ink group-hover:text-neon-pink sm:text-[15px]">{cert.title}</span>
                  <span className="ml-2 font-mono text-[10px] uppercase tracking-[0.1em] text-ink-mute">{cert.platform}</span>
                  {cert.score && <span className="ml-2 font-mono text-[9.5px] uppercase tracking-[0.08em] text-neon-pink">{cert.score}</span>}
                </div>
                <span className="flex flex-shrink-0 items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.14em] text-ink-mute group-hover:text-neon-cyan-bright">
                  {cert.year}
      ...
      {showOther && (
        <motion.div variants={neonReveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }}>
          <h3 className="mb-6 font-display text-xl font-bold uppercase tracking-wide text-ink">
            {activeFilter === 'All' ? '[ALL] other entries' : `[${activeFilter}]`}
          </h3>

          <div className="divide-y divide-white/5 border-y border-neon-cyan/20">
            {(showAll ? visibleOther : visibleOther.slice(0, 12)).map((cert) =>
              cert.url ? (
                ...
                >
                  <span className="truncate font-mono text-[13.5px] text-ink group-hover:text-neon-pink">{cert.title}</span>
                  <span className="hidden truncate font-mono text-[10px] uppercase tracking-[0.08em] text-ink-mute sm:block">{cert.platform}</span>
                  <span className="flex items-center gap-1.5 whitespace-nowrap font-mono text-[10px] uppercase tracking-[0.1em] text-ink-mute group-hover:text-neon-cyan-bright">
                    {cert.year}
                    <ExternalLink className="h-3 w-3" />
                  </span>
                </a>
                ...
                <div key={cert.title} className="grid grid-cols-[1fr_auto] items-baseline gap-3 py-3 sm:grid-cols-[2.4fr_1fr_auto]">
                  <span className="truncate font-mono text-[13.5px] text-ink-mute">{cert.title}</span>
          ...
          {visibleOther.length > 12 && (
            <div className="mt-8 flex justify-center">
              <TermButton variant="outline" onClick={() => setShowAll(!showAll)}>
                {showAll ? 'SHOW_LESS' : `VIEW_ALL (${visibleOther.length})`}
              </TermButton>
            </div>
          )}
        </motion.div>
