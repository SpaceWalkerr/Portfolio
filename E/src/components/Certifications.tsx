...
import { CaseSection, SectionHeader, CaseButton, caseReveal } from './ui/casefile';
...
const Certifications = () => {
  ...
  return (
    <CaseSection id="certifications" deep>
      <SectionHeader file="File 07 of 08" status="Status: Cross-Referenced" headline="Evidence Log" standfirst="Validated skills and milestones, indexed by category — sixty-plus entries logged since 2023." />

      <div className="mb-10 flex flex-wrap gap-2 border-y-2 border-ink/30 py-4">
        {categories.map((category) => (
          <button
            key={category}
            type="button"
            onClick={() => setActiveFilter(category)}
            className={`px-3.5 py-1.5 font-mono text-[9.5px] uppercase tracking-[0.1em] transition-colors ${
              activeFilter === category ? 'bg-blood text-paper' : 'border-2 border-ink/30 text-ink-mute hover:border-ink hover:text-ink'
            }`}
          >
            {category}
          </button>
      ...
      {showFeatured && (
        <motion.div variants={caseReveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }} className="mb-14">
          <h3 className="mb-1 font-display text-xl uppercase tracking-tight text-ink">Priority Evidence</h3>
          ...
          <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
            {notableAchievements.map((item, i) => (
              <div key={item.title} className="flex gap-4 border-t-2 border-ink/60 pt-3">
                <span className="font-display text-lg text-blood">{`0${i + 1}`}</span>
                <div>
          ...
          <div className="mt-10 divide-y-2 divide-ink/10 border-y-2 border-ink/30">
            {featuredCerts.map((cert) => (
              ...
              >
                <div className="min-w-0">
                  <span className="font-mono text-[14px] text-ink group-hover:text-blood sm:text-[15px]">{cert.title}</span>
                  <span className="ml-2 font-mono text-[10px] uppercase tracking-[0.1em] text-ink-mute">{cert.platform}</span>
                  {cert.score && <span className="ml-2 font-mono text-[9.5px] uppercase tracking-[0.08em] text-blood">{cert.score}</span>}
                </div>
      ...
      {showOther && (
        <motion.div variants={caseReveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }}>
          <h3 className="mb-6 font-display text-xl uppercase tracking-tight text-ink">
            {activeFilter === 'All' ? 'All Other Entries' : activeFilter}
          </h3>

          <div className="divide-y-2 divide-ink/10 border-y-2 border-ink/30">
            {(showAll ? visibleOther : visibleOther.slice(0, 12)).map((cert) =>
              cert.url ? (
                ...
                >
                  <span className="truncate font-mono text-[13.5px] text-ink group-hover:text-blood">{cert.title}</span>
          ...
          {visibleOther.length > 12 && (
            <div className="mt-8 flex justify-center">
              <CaseButton variant="outline" onClick={() => setShowAll(!showAll)}>
