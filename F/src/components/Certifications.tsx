...
import { FieldSection, SectionHeader, FieldButton, fieldReveal } from './ui/fieldguide';
...
const Certifications = () => {
  ...
  return (
    <FieldSection id="certifications" deep>
      <SectionHeader plate="Plate No. 7" collected="Verified Findings" headline="Findings &amp; Records" standfirst="Validated skills and milestones, catalogued by category — sixty-plus findings logged since 2023." />
      ...
      <div className="mb-10 flex flex-wrap gap-2 border-y border-ink/30 py-4">
        {categories.map((category) => (
          <button
            ...
            className={`px-3.5 py-1.5 font-serif text-[12px] italic transition-colors ${
              activeFilter === category ? 'bg-forest text-paper' : 'border border-ink/30 text-ink-mute hover:border-ink hover:text-ink'
            }`}
          >
            {category}
          </button>
      ...
      {showFeatured && (
        <motion.div variants={fieldReveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }} className="mb-14">
          <h3 className="mb-1 font-display text-xl italic text-ink">Notable Findings</h3>
          <p className="mb-6 font-serif text-sm italic text-ink-mute">The most significant entries in the record.</p>

          <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
            {notableAchievements.map((item, i) => (
              <div key={item.title} className="flex gap-4 border-t border-ink/50 pt-3">
                <span className="font-display text-lg italic text-forest">{`0${i + 1}`}</span>
                <div>
                  <h4 className="font-serif text-sm font-bold italic text-ink">{item.title}</h4>
                  <p className="mt-1 font-serif text-[13px] italic leading-snug text-ink-mute">{item.description}</p>
                </div>
              </div>
            ))}
          </div>

          <div className="mt-10 divide-y divide-ink/10 border-y border-ink/30">
            {featuredCerts.map((cert) => (
              ...
              >
                <div className="min-w-0">
                  <span className="font-serif text-[15px] italic text-ink group-hover:text-forest">{cert.title}</span>
                  <span className="ml-2 font-serif text-[11px] uppercase tracking-[0.06em] text-ink-mute">{cert.platform}</span>
                  {cert.score && <span className="ml-2 font-serif text-[11px] italic text-forest">{cert.score}</span>}
                </div>
                <span className="flex flex-shrink-0 items-center gap-1.5 font-serif text-[11px] uppercase tracking-[0.06em] text-ink-mute group-hover:text-ink">
                  {cert.year}
      ...
      {showOther && (
        <motion.div variants={fieldReveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }}>
          <h3 className="mb-6 font-display text-xl italic text-ink">{activeFilter === 'All' ? 'All Other Entries' : activeFilter}</h3>

          <div className="divide-y divide-ink/10 border-y border-ink/30">
            {(showAll ? visibleOther : visibleOther.slice(0, 12)).map((cert) =>
              cert.url ? (
                ...
                >
                  <span className="truncate font-serif text-[14px] italic text-ink group-hover:text-forest">{cert.title}</span>
                  <span className="hidden truncate font-serif text-[11px] uppercase tracking-[0.04em] text-ink-mute sm:block">{cert.platform}</span>
                  <span className="flex items-center gap-1.5 whitespace-nowrap font-serif text-[11px] uppercase tracking-[0.06em] text-ink-mute group-hover:text-ink">
                    {cert.year}
                    <ExternalLink className="h-3 w-3" />
                  </span>
                </a>
                ...
                <div key={cert.title} className="grid grid-cols-[1fr_auto] items-baseline gap-3 py-3 sm:grid-cols-[2.4fr_1fr_auto]">
                  <span className="truncate font-serif text-[14px] italic text-ink-mute">{cert.title}</span>
                  <span className="hidden truncate font-serif text-[11px] uppercase tracking-[0.04em] text-ink-mute/60 sm:block">{cert.platform}</span>
                  <span className="whitespace-nowrap font-serif text-[11px] uppercase tracking-[0.06em] text-ink-mute/60">{cert.year}</span>
                </div>
              )
            )}
          </div>
          ...
          {visibleOther.length > 12 && (
            <div className="mt-8 flex justify-center">
              <FieldButton variant="outline" onClick={() => setShowAll(!showAll)}>
