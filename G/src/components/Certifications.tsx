...
import { MissionSection, SectionHeader, MissionButton, missionReveal } from './ui/mission';
...
const Certifications = () => {
  ...
  return (
    <MissionSection id="certifications" deep>
      <SectionHeader log="Mission Log 07" status="Status: Archived" headline="Flight Records" standfirst="Validated skills and milestones, filed by category — sixty-plus entries logged since 2023." />

      <div className="mb-10 flex flex-wrap gap-2 border-y border-ink/20 py-4">
        {categories.map((category) => (
          <button
            key={category}
            type="button"
            onClick={() => setActiveFilter(category)}
            className={`px-3.5 py-1.5 font-mono text-[9.5px] uppercase tracking-[0.08em] transition-colors ${
              activeFilter === category ? 'bg-amber text-void-deep' : 'border border-ink/25 text-ink-mute hover:border-ink hover:text-ink'
            }`}
          >
            {category}
          </button>
      ...
      {showFeatured && (
        <motion.div variants={missionReveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }} className="mb-14">
          <h3 className="mb-1 font-display text-lg text-ink">Priority Records</h3>
          <p className="mb-6 font-mono text-xs text-ink-mute">The highest-impact entries on file.</p>

          <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
            {notableAchievements.map((item, i) => (
              <div key={item.title} className="flex gap-4 border-t border-ink/20 pt-3">
                <span className="font-mono text-lg text-amber">{`0${i + 1}`}</span>
                <div>
          ...
          <div className="mt-10 divide-y divide-white/5 border-y border-ink/20">
            {featuredCerts.map((cert) => (
              ...
              >
                <div className="min-w-0">
                  <span className="font-mono text-[14px] text-ink group-hover:text-amber sm:text-[15px]">{cert.title}</span>
                  <span className="ml-2 font-mono text-[10px] uppercase tracking-[0.1em] text-ink-mute">{cert.platform}</span>
                  {cert.score && <span className="ml-2 font-mono text-[9.5px] uppercase tracking-[0.08em] text-amber">{cert.score}</span>}
                </div>
      ...
      {showOther && (
        <motion.div variants={missionReveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }}>
          <h3 className="mb-6 font-display text-lg text-ink">{activeFilter === 'All' ? 'All Other Entries' : activeFilter}</h3>

          <div className="divide-y divide-white/5 border-y border-ink/20">
            {(showAll ? visibleOther : visibleOther.slice(0, 12)).map((cert) =>
              cert.url ? (
                ...
                >
                  <span className="truncate font-mono text-[13.5px] text-ink group-hover:text-amber">{cert.title}</span>
          ...
          {visibleOther.length > 12 && (
            <div className="mt-8 flex justify-center">
              <MissionButton variant="outline" onClick={() => setShowAll(!showAll)}>
