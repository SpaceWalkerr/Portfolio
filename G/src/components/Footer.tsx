...
const Footer = () => {
  ...
  return (
    <footer className="border-t border-ink/20 bg-void-deep text-ink">
      <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 sm:py-16 lg:px-12">
        <div className="mb-8 font-mono text-[10px] uppercase tracking-[0.3em] text-amber">Mission Complete &middot; SN-2026</div>

        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr] lg:gap-8">
          <div>
            <span className="font-display text-lg text-ink">Suraj Nandan</span>
            ...
            <div className="mt-5 flex items-center gap-2">
              {socialLinks.map((social) => (
                <a key={social.label} href={social.url} target="_blank" rel="noopener noreferrer" aria-label={social.label} className="border border-ink/25 p-2.5 text-ink transition-colors hover:border-amber hover:text-amber">
                  {social.icon}
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <a href={link.href} onClick={(e) => scrollToSection(e, link.href)} className="font-mono text-[13px] text-ink hover:text-amber">
                    {link.label}
                  </a>
                </li>
              ))}
          ...
          <div>
            <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-ink-mute">Comms</span>
            <ul className="mt-4 space-y-2 font-mono text-[13px]">
              <li>
                <a href="mailto:surajnandan78@gmail.com" className="text-ink hover:text-amber">
                  surajnandan78@gmail.com
                </a>
              </li>
              <li>
                <a href="tel:+916203484989" className="text-ink hover:text-amber">
                  +91 6203484989
                </a>
              </li>
              <li className="text-ink-mute">Kishanganj, Bihar, India</li>
            </ul>
            <button type="button" onClick={() => setResumeModalOpen(true)} className="mt-4 border border-ink/30 px-4 py-2 font-mono text-[10px] uppercase tracking-[0.14em] text-ink transition-colors hover:border-amber hover:text-amber">
              Flight Manifest
            </button>
          </div>
        </div>
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-3 border-t border-white/10 pt-6 font-mono text-[9px] uppercase tracking-[0.14em] text-ink-mute sm:flex-row">
          <p>&copy; {currentYear} Suraj Nandan &mdash; Mission Complete, Log 8 of 8</p>
          <p>Set in Michroma &amp; Share Tech Mono</p>
        </div>
      </div>
