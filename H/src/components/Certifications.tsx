...
import { ArcadeSection, SectionHeader, ArcadeButton, arcadeReveal } from './ui/arcade';
...
const Certifications = () => {
  ...
  return (
    <ArcadeSection id="certifications" deep>
      <SectionHeader world="World 1-7" status="Status: Unlocked" headline="Trophy Room" standfirst="Validated skills and milestones, indexed by category — sixty-plus trophies earned since 2023." />

      <div className="mb-10 flex flex-wrap gap-2 border-y-4 border-ink/40 py-4">
        {categories.map((category) => (
          <button
            key={category}
            type="button"
            onClick={() => setActiveFilter(category)}
            className={`px-3.5 py-1.5 font-mono text-sm uppercase transition-colors ${
              activeFilter === category ? 'bg-gold text-void-deep' : 'border-2 border-ink/40 text-ink-mute hover:border-ink hover:text-ink'
            }`}
          >
            {category}
          </button>
      ...
      {showFeatured && (
        <motion.div variants={arcadeReveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }} className="mb-14">
          <h3 className="mb-1 font-display text-sm uppercase text-ink">Rare Trophies</h3>
          <p className="mb-6 font-mono text-lg text-ink-mute">The highest-impact entries in the case.</p>

          <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
            {notableAchievements.map((item, i) => (
              <div key={item.title} className="flex gap-4 border-t-4 border-ink/40 pt-3">
                <span className="font-display text-base text-gold">{`0${i + 1}`}</span>
                <div>
                  <h4 className="font-mono text-base font-bold text-ink">{item.title}</h4>
                  <p className="mt-1 font-mono text-base leading-snug text-ink-mute">{item.description}</p>
                </div>
              </div>
            ))}
          </div>

          <div className="mt-10 divide-y-2 divide-white/5 border-y-2 border-ink/40">
            {featuredCerts.map((cert) => (
              ...
              >
                <div className="min-w-0">
                  <span className="font-mono text-lg text-ink group-hover:text-gold">{cert.title}</span>
                  <span className="ml-2 font-mono text-sm uppercase text-ink-mute">{cert.platform}</span>
                  {cert.score && <span className="ml-2 font-mono text-sm uppercase text-gold">{cert.score}</span>}
                </div>
                <span className="flex flex-shrink-0 items-center gap-1.5 font-mono text-sm uppercase text-ink-mute group-hover:text-ink">
                  {cert.year}
      ...
      {showOther && (
        <motion.div variants={arcadeReveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }}>
          <h3 className="mb-6 font-display text-sm uppercase text-ink">{activeFilter === 'All' ? 'All Other Entries' : activeFilter}</h3>

          <div className="divide-y-2 divide-white/5 border-y-2 border-ink/40">
            {(showAll ? visibleOther : visibleOther.slice(0, 12)).map((cert) =>
              cert.url ? (
                ...
                >
                  <span className="truncate font-mono text-lg text-ink group-hover:text-gold">{cert.title}</span>
                  <span className="hidden truncate font-mono text-sm uppercase text-ink-mute sm:block">{cert.platform}</span>
                  <span className="flex items-center gap-1.5 whitespace-nowrap font-mono text-sm uppercase text-ink-mute group-hover:text-ink">
                    {cert.year}
                    <ExternalLink className="h-3 w-3" />
                  </span>
                </a>
                ...
                <div key={cert.title} className="grid grid-cols-[1fr_auto] items-baseline gap-3 py-3 sm:grid-cols-[2.4fr_1fr_auto]">
                  <span className="truncate font-mono text-lg text-ink-mute">{cert.title}</span>
                  <span className="hidden truncate font-mono text-sm uppercase text-ink-mute/60 sm:block">{cert.platform}</span>
                  <span className="whitespace-nowrap font-mono text-sm uppercase text-ink-mute/60">{cert.year}</span>
                </div>
              )
            )}
          </div>
          ...
          {visibleOther.length > 12 && (
            <div className="mt-8 flex justify-center">
              <ArcadeButton variant="outline" onClick={() => setShowAll(!showAll)}>
